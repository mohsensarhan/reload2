  ...
  try {
    const response = await fetch('https://metabase.efb.eg/api/dataset', {
      ...
      body: JSON.stringify({
        ...
        "type": "native",
        "native": {
          "query": "SELECT id, amount_egp, amount_usd, currency, date, status FROM public.payment_donation WHERE status = 'S' ORDER BY date DESC LIMIT 1000"
        }
      })
    });
