...
// FRED API for Beef Prices
const FRED_BEEF_URL = 'https://fred.stlouisfed.org/graph/fredgraph.csv?id=PBEEFUSDM';

export default async function handler() {
  try {
    const response = await fetch(FRED_BEEF_URL, {
      headers: { 
    ...
    if (!response.ok) {
      return fail(response.status, `FRED Beef ${response.status}`);
    }
